...
def make_decoder(opt, embeddings):
    ...
    if opt.decoder_type == "transformer":
        ...
        return InputFeedRNNDecoder(opt.rnn_type, opt.brnn,
                                   opt.dec_layers, opt.rnn_size + opt.size_vae,
...
def make_latent_variable_LSTM(model_opt, fields, gpu, checkpoint=None):
    ...
    glb = nn.Linear(model_opt.rnn_size, model_opt.size_vae + model_opt.rnn_size)
    ...
    if not model_opt.copy_attn:
        generator = nn.Sequential(
            nn.Linear(model_opt.rnn_size + model_opt.size_vae, len(fields["tgt"].vocab)),
            nn.LogSoftmax())
        if model_opt.share_decoder_embeddings:
            generator[0].weight = decoder.embeddings.word_lut.weight
    else:
        generator = CopyGenerator(model_opt.rnn_size + model_opt.size_vae,
                                  fields["tgt"].vocab)
